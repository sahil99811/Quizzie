...
exports.getTrendingQuizzs = async (req, res) => {
    try {
        ...
        const totalQuestionsResult = await Quizz.aggregate([
            { $match: { createdBy: id } },
            { $group: { _id: null, totalQuestions: { $sum: { $size: "$questions" } } } }
        ]);
        ...
        res.status(201).json({
            ...
            totalImpressions: totalImpressions.length !== 0 ? totalImpressions[0]?.totalImpression : 0,
            totalQuizzes: totalQuizzes,
            totalQuestions: totalQuestionsResult.length !== 0 ? totalQuestionsResult[0]?.totalQuestions : 0,
            trendingQuizzes,
        });
    } catch (err) {
        console.error(err);
...
exports.getQuizzsAnalysis = async (req, res) => {
    try {
        const { id } = req.user;
        const quizzs = await Quizz.find({ createdBy: id }).select("quizzName impression createdAt _id");
...
exports.getQuizzAnalysis = async (req, res) => {
    try {
        const { id } = req.user;
...
exports.quizTest = async (req, res) => {
    try {
        ...
        // Send success response
        res.status(200).json({ success: true, message: 'Quiz questions fetched successfully', questions: quizzQuestions });
    } catch (error) {
        console.error(error);
        errorResponse(res, 500, 'Internal server error');
    }
